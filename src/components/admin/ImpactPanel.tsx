import { useSiteConfig, ImpactStoryItem } from "@/lib/siteConfig";
...
export default function ImpactPanel() {
  ...
  const set = (patch: Partial<typeof im>) => updateConfig({ impact: { ...im, ...patch } });

  const updateStory = (id: string, patch: Partial<ImpactStoryItem>) => {
    set({ stories: im.stories.map((s) => s.id === id ? { ...s, ...patch } : s) });
  };
  ...
  const removeStory = (id: string) => {
    const updated = im.stories.filter((s) => s.id !== id);
    set({
      stories: updated,
      featuredStoryId: im.featuredStoryId === id ? (updated[0]?.id ?? "") : im.featuredStoryId,
    });
  };

  const featuredStory = im.stories.find((s) => s.id === im.featuredStoryId);

  return (
    <div className="space-y-8">
      {/* Header */}
      <div>
        <h2 className="text-lg font-display font-bold text-foreground mb-1">Impact</h2>
        <p className="text-sm text-muted-foreground">Edit headlines, metrics, chart data, and stories from the field.</p>
      </div>

      {/* Hero Config */}
      <div className="space-y-4">
        <Field label="Hero Title" value={im.heroTitle} onChange={(v) => set({ heroTitle: v })} />
        <Field label="Hero Subtitle" value={im.heroSubtitle} onChange={(v) => set({ heroSubtitle: v })} multiline />
        <div className="grid grid-cols-3 gap-4">
          <NumField label="Meals Served" value={im.mealsServed} onChange={(v) => set({ mealsServed: v })} />
          <NumField label="Beneficiaries" value={im.beneficiaries} onChange={(v) => set({ beneficiaries: v })} />
          <NumField label="Projects" value={im.projectsCompleted} onChange={(v) => set({ projectsCompleted: v })} />
        </div>
      </div>
      ...
      <div>
        <div className="flex items-center justify-between mb-3">
          <h3 className="text-sm font-bold text-foreground">Bar Chart Data (Meals by Quarter)</h3>
          <button onClick={() => set({ chartData: [...im.chartData, { quarter: "", meals: 0 }] })}
            className="inline-flex items-center gap-1 text-xs text-primary font-semibold hover:underline">
            <Plus className="w-3.5 h-3.5" /> Add
          </button>
        </div>
        <div className="space-y-3">
          {im.chartData.map((d, i) => (
            <div key={i} className="flex items-end gap-3 bg-muted rounded-lg p-3">
              <div className="flex-1"><Field label="Quarter" value={d.quarter} onChange={(v) => {
                const chartData = [...im.chartData]; chartData[i] = { ...chartData[i], quarter: v }; set({ chartData });
              }} /></div>
              <div className="flex-1"><NumField label="Meals" value={d.meals} onChange={(v) => {
                const chartData = [...im.chartData]; chartData[i] = { ...chartData[i], meals: v }; set({ chartData });
              }} /></div>
              <button onClick={() => set({ chartData: im.chartData.filter((_, idx) => idx !== i) })}
                className="text-destructive hover:text-destructive/80 mb-1" aria-label="Remove data point">
      ...
      <div className="bg-muted rounded-xl p-5 space-y-3">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="text-sm font-bold text-foreground">Stories Hero Background</h3>
        </div>
        ...
        <ImageUploadField label="Background Image" value={im.heroBackgroundImage} onChange={(v) => set({ heroBackgroundImage: v })} previewHeight="h-32" />
      </div>
      ...
      <div className="bg-gradient-to-br from-primary/5 to-primary/10 border border-primary/20 rounded-xl p-5 space-y-3">
        ...
        <p className="text-xs text-muted-foreground">This story appears as the large hero banner in the "Stories from the Field" section.</p>
        <select
          value={im.featuredStoryId}
      ...
      <div>
        <div className="flex items-center justify-between mb-4">
          <div>
            <h3 className="text-sm font-bold text-foreground">Stories from the Field ({im.stories.length})</h3>
        ...
        <div className="space-y-4">
          {im.stories.map((s) => {
            const isFeatured = s.id === im.featuredStoryId;
            return (
              <div key={s.id}
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                  ...
                  <div className="sm:col-span-2">
                    <Field label="Excerpt" value={s.excerpt} onChange={(v) => updateStory(s.id, { excerpt: v })} multiline placeholder="Brief summary..." />
                  </div>
                  <div className="sm:col-span-2">
                    <Field label="Full Content" value={s.fullContent} onChange={(v) => updateStory(s.id, { fullContent: v })} multiline placeholder="Full story text (supports paragraphs separated by blank lines)..." />
                  </div>
                  <div>
                    <div className="space-y-1">
